...
/** Per-org AI configuration: provider choice, encrypted API key, model
 *  routing. The key is AES-256-GCM at rest and never leaves the server. */
export const aiSettings = pgTable(
  "ai_settings",
  {
    ...
    orgId: uuid("org_id")
      .notNull()
      .references(() => organizations.id, { onDelete: "cascade" }),
    provider: text("provider").notNull().default("nim"), // nim | openrouter | groq | mistral | zai
    encryptedApiKey: text("encrypted_api_key"),
    keyLast4: text("key_last4"),
    baseUrl: text("base_url"),
    /** Role-based routing: { primary, fast, reasoning, embeddings, ocr }. */
    modelRouting: jsonb("model_routing").notNull().default({}),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [uniqueIndex("ai_settings_org_idx").on(t.orgId)],
);
