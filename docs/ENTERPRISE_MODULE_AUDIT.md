...
Implementation: the first occurrence-integrity slice in ADR 0045 makes scheduled claim, unique `(routineId,scheduledOccurrence)` creation, rescheduling, and job enqueue one transaction; it records the occurrence/job link and updates the occurrence outcome when execution finishes. Remaining work is lease/fencing integration at the occurrence level, a scoped mandate with owner, allowed reads/actions, recipient audience and expiry, suspension/module/permission rechecks, explicit completed-no-findings/findings/partial/blocked/failed/cancelled outcomes, disabled-run policy, and designated authorized recipients instead of broad routine broadcasts.
