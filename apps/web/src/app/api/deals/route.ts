...
import { z } from "zod";
import { deals, getDb } from "@chaste/db";
...
export async function GET() {
  ...
  return NextResponse.json({
    deals: rows.map((d) => ({ ...d, createdAt: d.createdAt.toISOString(), updatedAt: d.updatedAt.toISOString() })),
  });
}
...
export async function POST(req: Request) {
  ...
  if (body.data.action === "create") {
    const result = await executor.execute(
      "crm.createDeal",
      humanCtx,
      { title: body.data.title, valueMinor: body.data.valueMinor },
    );
